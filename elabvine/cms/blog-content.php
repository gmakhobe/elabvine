...
if (isset($_REQUEST["Id"]) && isset($_REQUEST["title"])) {
    ...
    //sql
    $sql = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM blog WHERE id = '$id' AND title = '$title' AND source = 'elabvine';");
    if (mysqli_num_rows($sql) > 0) {
        $row = mysqli_fetch_array($sql);
        $hits = $row["hits"] + 1;
        $sql_update = "UPDATE blog SET hits = '$hits' WHERE id = '$id' AND source = 'elabvine';";
