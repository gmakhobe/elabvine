...
session_start();
//Session or cookie
if (isset($_SESSION['session_learnlive']) || isset($_COOKIE['remember_me_learnlive'])) {

    if (isset($_SESSION['session_learnlive'])) {
        $email_address = $_SESSION['session_learnlive'];
    } else if (isset($_COOKIE['remember_me_learnlive'])) {
        $_SESSION['session_learnlive'] = $_COOKIE['remember_me_learnlive'];
        $email_address = $_SESSION['session_learnlive'];
    } else {
    ...
    if (mysqli_num_rows($sql) > 0) {
        ...
        //Table members_...
        //Get Info
        $sql_2 = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM `learnlive_members_personal_information` where Member_Id = '$user_id';");
