...
#sql
            $sql = "INSERT INTO subscribers(name, email, source, date) VALUES('$name', '$emailAddress', '$source', '$currentDateTime');";
#execute
...
//select email
            if ($sql2 = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM " . $companyuri . "_members_information where Email_Address = '$email';")) {
                ...
                session_start();
                $_SESSION['session_learnlive'] = $email;
    ...
    case "Login":
        ...
        if (mysqli_num_rows($sql) > 0) {
            ...
            if ($remember == "yes") {
                ...
                $cookie_value = $_SESSION['session_learnlive'];
...
//Student First Step Info
                ...
                //Seen if table exists'
                $sql = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM `learnlive_members_personal_information` WHERE Member_Id = '$query_cookie';");
                if (mysqli_num_rows($sql) > 0) {
                    //multi sql
                    ...
                    $sql .= "UPDATE `learnlive_members_personal_information` SET Date_Birth = '$datebirth', Gender = '$gender', Grade = '$grade', School = '$school' WHERE Member_Id = '$query_cookie';";
//multi sql exec
                    ...
                    //multi sql
                    $sql = "UPDATE " . $companyuri . "_members_information SET Name = '$name', Surname = '$surname', Account_Type = 'Student', Account_Status = 'Step 2' WHERE Id = '$query_cookie';";
                    $sql .= "INSERT INTO `learnlive_members_personal_information`(Member_Id, Date_Birth, Gender, Grade, School) VALUES('$query_cookie', '$datebirth', '$gender', '$grade', '$school');";
...
//Student Address Info
                ...
                $address5 = mysqli_real_escape_string($DATABASE_CONNECTION, $_POST['address5']);
//multi sql
                $sql = "UPDATE `learnlive_members_personal_information` SET Address_Street = '$address1', Address_Suburb = '$address2', Address_City = '$address3', Address_Zip = '$address4', Province = '$address5' WHERE Member_Id = '$query_cookie';";
...
//Student First Step Info
                ...
                $email = mysqli_real_escape_string($DATABASE_CONNECTION, $_POST['email']);
//multi sql
                $sql = "UPDATE `learnlive_members_personal_information` SET Guardian_Name = '$name', Guardian_Phone = '$phone', Guardian_Email = '$email' WHERE Member_Id = '$query_cookie';";
...
//Student First Step Info
                ...
                if (mysqli_num_rows($sql) > 0) {
                    ...
                    $sql .= "UPDATE `learnlive_members_personal_information` Id_Number = '$identification', Gender = '$gender', Occupation = '$occupation' WHERE Member_Id = '$query_cookie';";
//multi sql exec
                    if (mysqli_multi_query($DATABASE_CONNECTION, $sql)) {
                        $results = "success";
                    } else {
                    ...
                    //multi sql
                    $sql = "UPDATE " . $companyuri . "_members_information SET Name = '$name', Surname = '$surname', Account_Type = 'Tutor', Account_Status = 'Step 2' WHERE Id = '$query_cookie';";
                    $sql .= "INSERT INTO `learnlive_members_personal_information`(Member_Id, Id_Number, Gender, Occupation) VALUES('$query_cookie', '$identification', '$gender', '$occupation');";
//multi sql exec
                    if (mysqli_multi_query($DATABASE_CONNECTION, $sql)) {
                        $results = "success";
                    } else {
...
//Fetch rows
        ...
        } else if ($source == "SetProfileNotification") {

            $sql_acc = mysqli_query($DATABASE_CONNECTION, "SELECT Account_Type FROM " . $companyuri . "_members_information WHERE Id = '$query_cookie' AND Account_Type = 'Tutor';");
            if (mysqli_num_rows($sql_acc) > 0) {

//sql
                $sql = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM `scimatix_tutor_profiles` WHERE Member_Id = '$query_cookie';");
                if (mysqli_num_rows($sql) > 0) {
                    ...
                    $results = '<div class="alert alert-warning alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    <center>You need to upload your tutoring profile as soon as possible...</center>
                  </div>';
                }
            } else {
                $results = "";
            }
        } else if ($source == "MessageNotification") {
//include time ago
            include("timeago.php");

            $sql_acc = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM `scimatix_notification` WHERE Member_Id = '$query_cookie' AND Status = 'Waiting';");
            if (mysqli_num_rows($sql_acc) > 0) {
                $results = '';
                while ($row = mysqli_fetch_array($sql_acc, MYSQLI_ASSOC)) {
                    $results = '<div class="alert alert-info alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    Title: ' . $row["Title"] . '<br>
                    Message: ' . $row["Message"] . '<br>
					Posted: ' . facebook_time_ago($row["Datetime"]) . '
					<center><button onclick="markAsSeen(' . $row["Id"] . ')" class="btn btn-success btn-sm"><i class="fa fa-magic"></i>&nbsp; Mark As Seen</button></center>
                  </div>';
                }
            } else {
                $results = "";
...
//naming
            ...
            if (mysqli_num_rows($sql) > 0) {
                ...
                $sql_insert = "UPDATE `" . $companyuri . "_tutor_profiles` SET Status = 'Published', Language_One = '$language1', Language_Two = '$language2', Student_Number = '$studentNumber', Degree = '$degree', Short_Course = '$shortCourse', Employer = '$employer', Position = '$position', Period_Employed = '$periodEmployed', Subjects = '$grades', Grades = '$subjects', Id_Documentation = '$new_id', CV_Documentation = '$new_cv', Date_Posted = '$now' WHERE Member_Id = '$query_cookie';";
...
//Query Cookie
        $query_cookie = $_COOKIE['query_cookie'];
//Get values
        $source = mysqli_real_escape_string($DATABASE_CONNECTION, $_REQUEST['id']);
        $sql = "UPDATE `scimatix_notification` SET Status = 'Seen' WHERE Id = '$source';";
        if (mysqli_query($DATABASE_CONNECTION, $sql)) {
            $results = 'success';
        } else {
            $results = 'error';
