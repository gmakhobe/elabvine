...
if (!isset($_REQUEST["reqId"])) {
    if ($account_type = "Student") {
        $display = "includes/lessons-menu.php";
    }
} else {
    if(isset($_REQUEST["reqId"]) && $_REQUEST["reqId"] == "reportStudent") {
         $display = "includes/lessons-report.php";
    } else {
        $display = "includes/lessons-manager.php";
        //Id
        $lessonId = mysqli_real_escape_string($DATABASE_CONNECTION, $_REQUEST["reqId"]);
        #Sql
        $sql = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM `" . $companyuri . "_members_lessons` WHERE Lesson_Id = '$lessonId';");
        if (mysqli_num_rows($sql) > 0) {
            ...
            if ($sql2 = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM `" . $companyuri . "_members_information` WHERE Id = '$student_id';")) {
                $row2 = mysqli_fetch_array($sql2, MYSQLI_ASSOC);

                $student_name = $row2['Name'];
                $student_surname = $row2['Surname'];

                if ($sql3 = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM `members_personal_information` WHERE Member_Id = '$student_id';")) {
                    $row3 = mysqli_fetch_array($sql3, MYSQLI_ASSOC);
                    $student_profile = $row3['Profile_Picture'];
                    if ($student_profile == "") {
                        $student_profile = "../img/profile.png";
                    }
                }
            }
        }
...
<html lang="zxx" class="no-js">
    <head>
        ...
        <title>Lessons</title>
        <!-- Meta Description -->
        <meta name="description" content="<?php echo $companyuri; ?> Lessons">      
        <!-- Header -->
            ...
            <div class="container">				
                <div class="row d-flex align-items-center justify-content-center">
                    <div class="about-content col-lg-12">
                        ...
                        <p class="text-white link-nav"><a href="index.php">Dashboard </a>  <span class="lnr lnr-arrow-right"></span>  <a href="lessons.php"> Lessons</a></p>
                    </div>		
                </div>
            </div>
        </section>
