...
switch ($_REQUEST['reqId']) {
    case "page-blog":
        ...
        $sql = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM blog WHERE source = 'elabvine' ORDER BY id DESC
LIMIT 0, 3;");
//If success
    ...
    case "page-index":
         ...
         * * */
        //Select blogs where source is elab
        $sql = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM blog WHERE source = 'elabvine' ORDER BY date DESC
LIMIT 0 , 3;");
//If success
        if (mysqli_num_rows($sql)) {
            //Start blog row
            ...
            while ($row = mysqli_fetch_array($sql)) {
                ...
                <div class="single-recent-blog col-lg-4 col-md-4">
                    <div class="thumb">
                        <img class="f-img img-fluid mx-auto" src="' . $row["image_link"] . '" alt="">	
                    </div>
                    <div class="bottom d-flex justify-content-between align-items-center flex-wrap">
                        <div>
                            <img class="img-fluid" style="width:80px !important;" src="' . $row["author_image_link"] . '" alt="">
                            <a href="blog-content.php?Id=' . $row["id"] . '&title=' . $row["title"] . '"><span>' . $row["author"] . '</span></a>
                        </div>
                        <div class="meta">
                           ' . $showDateTime . '     
                        </div>
                   </div>							
                   <a href="blog-content.php?Id=' . $row["id"] . '&title=' . $row["title"] . '">
                        <h4>' . $row["title"] . '</h4>
                   </a>
                   <p>
                        ' . $row["summary"] . '
    ...
    case "page-blog-popular":
         ...
         * * */
        //Select blogs where source is elab
        $sql = mysqli_query($DATABASE_CONNECTION, "SELECT * FROM blog WHERE source = 'elabvine' ORDER BY hits DESC
LIMIT 0 , 3;");
//If success
        if (mysqli_num_rows($sql)) {
            //Start blog row
