...
function seen_notifications() {
    ...
    xhttp.open("POST", "notifications.php", true);
    xhttp.send(fd);
}
$(document).ready(function () {
    $(".search").hide();
...
function showHint(str) {
    var xhttp;
    if (str.length == 0) {
        document.getElementById("search").innerHTML = "Type something...";
        $(".search").hide();
        return;
    }
...
function saveProfile() {
    ...
    if (namesurname == "" || accounttype == "" || countries == "" || cellnumber == "" || password == "") {
...
function load_unseen_gigs() {
    ...
    xmlhttp.open("POST", "fetch-gigs.php", true);
    xmlhttp.send(fd);
}
